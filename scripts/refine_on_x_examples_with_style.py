...
from promptsmith.tasks.bulletize_text import BulletizeTextWithStyle  # <-- this is now the *module*, not signature
...
def setup_refinement_orchestrator(style, max_iterations=5, score_threshold=0.90):
    """Set up the refinement orchestrator with judges and bulletizer using the given style."""
    
    dspy, lm = get_dspy()

    bulletize_text_evaluator = TaskEvaluator(
        task=dspy.ChainOfThought(BulletizeTextWithStyle(style=style)),
        judges={
...
def main():
    ...
    parser.add_argument('--style', type=str, default='default',
                       help='Bulletization style (default: "default")')
    ...
    orchestrator = setup_refinement_orchestrator(style, max_iterations, score_threshold)
