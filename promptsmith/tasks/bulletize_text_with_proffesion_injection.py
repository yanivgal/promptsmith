...
def BulletizeTextWithProfession(proffesion="technical"):

    proffesion_prompt = f"You are an expert {proffesion} writer."

    base_doc = BulletizeText.__doc__ or ""

    full_doc = proffesion_prompt + "\n\n" + base_doc

    return type(
        f"BulletizeText_{proffesion}",
        (dspy.Signature,),
        {
            "__doc__": full_doc,
